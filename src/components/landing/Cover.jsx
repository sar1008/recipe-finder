...
export function Cover() {
  ...
  return (
    <div className="mb-10 mt-28 flex w-full max-w-screen-xl flex-row max-sm:flex-col max-sm:items-center max-sm:justify-center">
      ...
      <div className="mr-20 grid w-1/2 grid-cols-2 gap-4 max-sm:mr-0 max-sm:w-full">
        {images.map((image, index) => (
          <div key={index} className="relative">
            <img src={image.src} alt={image.label} className="h-auto w-full" />
            <div className="absolute bottom-0 left-0 right-0 flex justify-center bg-white bg-opacity-75">
              <div className="rounded-md px-2 py-1 text-sm">{image.label}</div>
            </div>
          </div>
        ))}
      </div>
